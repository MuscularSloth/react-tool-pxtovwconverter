...
const SingleValuePage = () => {
  ...
  const [selectedWidth, setSelectedWidth] = useLocalStorage<number>(
    'selectedWidth',
    initialSelectedWidthValue,
  );
  const [customPresetWidth, setCustomPresetWidth] = useLocalStorage<number[]>(
    'customPresetWidth',
    initialCustomPresetWidth,
  );
  ...
  const [presetWidth, setPresetWidth] = useState<number[]>(presetViewportWidth);
  ...
  const handleCalculateClick = async () => {
    ...
    if (calculatedValueAsFloat <= 0 || Number.isNaN(calculatedValueAsFloat)) {
    ...
    if (isAutoCopyOn) {
      try {
        ...
        toast.success(`Result has been copied! - ${result}vw`);
      } catch (error) {
